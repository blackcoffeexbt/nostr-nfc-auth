...
nostrnfcauth_generic_router = APIRouter()


def nostrnfcauth_renderer():
    return template_renderer(["nostrnfcauth/templates"])


@nostrnfcauth_generic_router.get("/", response_class=HTMLResponse)
async def index(request: Request, user: User = Depends(check_user_exists)):
    return nostrnfcauth_renderer().TemplateResponse(
        "nostrnfcauth/index.html", {"request": request, "user": user.dict()}
    )


@nostrnfcauth_generic_router.get("/{card_id}", response_class=HTMLResponse)
